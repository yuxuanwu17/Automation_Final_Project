...
@timeit
def KNNModel(data, method="one-time", initial=2000, once_add=1000, query_method="random", Beta=1):
    ...
    if method == "one-time":
        ...
        print(table)
        with open(f'results/KNN_{query_method}_initial_{initial}_batchsize_{once_add}_Beta{Beta}_K8_metricminkowski_p2.csv', 'w', newline='') as f_output:
...
if __name__ == "__main__":
    ...
    KNNModel(data=data, method=results.method, query_method=results.query_method, once_add=results.once_add)
